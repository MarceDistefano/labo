...
PARAM  <- list()
PARAM$experimento  <- "CA6112Estadistico"
...
#}
CorregirCampoMes <- function(pcampo, pmeses) {
  tbl <- dataset[, list("v1" = mean(c(v2[1:3], v1, v2[5:7]))), by = foto_mes][, v2 := NULL]
  
  dataset[foto_mes %in% pmeses, paste0(pcampo) := get(pcampo)]
  dataset[!foto_mes %in% pmeses, paste0(pcampo) := tbl$v1]
}
